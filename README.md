...
图片元数据与路径分离存储的 Web 管理系统：支持上传、BLOB 迁移、**BLOB 数据浏览**（目录树 + 表数据 + SQL 查询 + 多 BLOB 预览）。
...
| 指纹对比 | 成对指纹入库、筛选、多层特征叠加对比 | 所有登录用户 |
| BLOB 迁移 | 从旧库 BLOB 字段导出到 `upload/` 并写入路径表 | 所有登录用户 |
| BLOB 数据浏览 | 目录树浏览远程库、表/数据库视图数据、SQL 查询与多 BLOB 预览 | 所有登录用户 |
| 分类管理 | 维护图片分类 | 所有登录用户 |
